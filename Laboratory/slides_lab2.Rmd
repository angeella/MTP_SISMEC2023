...
In hypothesis testing, the probability of committing a Type I error is controlled at a set level $\alpha$, conventionally 5%. However, a problem arises when 
we conduct not just one test, but more than one. Since for each test we have a probability of committing a type I error, if we conduct many tests it is likely to get at least one type I error. 

The multiple testing problem has a simple structure. We have a set of null hypotheses
$\mathcal{H}=(H_1,\ldots,H_m)$, of which an unknown number $m_0$ are true, while the remaining $m-m_0$ are false. We denote the subset of true hypotheses by $\mathcal{T}\subseteq \mathcal{H}$, the subset of false hypotheses by $\mathcal{F}=\mathcal{H}\setminus \mathcal{T}$ and 
the proportion of true hypotheses with $\pi_0=m_0/m$. 

The goal of multiple verification procedures is to choose a subset $\mathcal{R}\subseteq \mathcal{H}$ of hypotheses to reject. Suppose we have available the p-values $p_1,\ldots,p_m$ for each hypothesis $H_1,\ldots,H_m$. We can consider the following procedure:
\[
...
Ideally, the set of rejected hypotheses $\mathcal{R}$ should coincide as closely as possible with the set of false hypotheses $\mathcal{F}$. Two types of errors can be made: type I errors $\mathcal{R}\cap \mathcal{T}$, that is, rejecting true hypotheses, and type II errors $\mathcal{R}\setminus \mathcal{F}$, that is, not rejecting false hypotheses. 
...
Using the procedure.
* Rejection $H_i$ if $p_i \leq \tilde{\alpha}$
...
We denote by $E_i=I\{P_i \leq \alpha}$ the outcome of the test, i.e., through the indicator function which is worth 1 if we reject the hypothesis, 0 otherwise. Since the hypotheses are all true, the p-values $P_1,\ldots,P_m$ are i.i.d. $\mathrm{Uniform}(0,1)$.
...
The case of 2 independent tests.
Empirical distribution of p-value pairs for 1000 hypothetical experiments  
```{r}
...
In the example of the drug that should 
...
Carlo Emilio Bonferroni was an Italian mathematician, best known for the inequalities that bear his name.
...
Let consider the `mtept` dataset:
...
It leads to the same waste as Bonferroni, sometimes a few more (uniformly more powerful).

It is iterative:

1. Correct the p-values with Bonferroni,
2. Reject all hypotheses with significant p-value
...
One test per pairwise comparison
...
> The empirical distribution of the number of rejections using the correction of Bonferroni, and Holm
