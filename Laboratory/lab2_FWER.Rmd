...
that is, Bonferroni's method controls the FWER at $\alpha$ (actually controls at a lower level, i.e., $\pi_0 \alpha$). 
...
Let us now consider *microarray* type data used in the study described by Golub et al. (1999). In this study, gene expression on $3051$ genes was measured for each patient. The $38$ patients considered are leukemia patients, and can be divided into two groups: 

1. $27$ patients with *acute lymphoblastic leukemia* (ALL, code `0`) and 
2. $11$ patients with *acute myeloid leukemia* (AML, code `1`). 

The aim of the study is to determine which genes are differentially expressed between the two groups of patients.
...
- the number of reflux episodes per hour (# EPS/HR), and 
...
In order to know if there is a dose response and which doses are effective, define an appropriate multiple testing procedure. For example:

- $\alpha/2$ is splitted among the three comparisons 
- the remaining $\alpha/2$ is allocated to the comparison 0 (placebo) vs 40 (i.e., $H_{40}$)
...
- $\alpha/2$ is splitted among the three comparisons, that is $\alpha/6$.  
- Since to $H_{40}$ is allocated an extra $\alpha/2$:  
    * $H_{40}$ are tested at level $\alpha/6+\alpha/2=4\alpha/6$  
    * $H_{20}$ and $H_{10}$ are tested at level $\alpha/6$  
- when $H_{40}$ is rejected,  $H_{20}$ is tested at level $\alpha/6+4\alpha/6=5\alpha/6$
- when $H_{20}$ is rejected,  $H_{10}$ is tested at level $\alpha/6+5\alpha/6=\alpha$

we don't see room for further improvements, the $\alpha$ is fulled exploited.


### 3.2 A procedure with three endpoints {-}
...
**Test the three endpoints sequentially:** that is,
- test % RT with procedure at point 3.1,  
- if $H_{10}$ is rejected (for endpoint % RT), perform the same procedure for \#EPS/HR,  
- only if  $H_{10}$ is rejected for endpoint  \#EPS/HR,  perform the same procedure for \#EPS > 	5'/12HR 

**Test the three endpoints in parallel:** that is,
use procedure at point 3.1 at $\alpha$s levels reduced by 1/3. As an example,  $H_{40}$ in each endpoints is tested at level $4\alpha/6/3=4\alpha/18=2\alpha/9$ etc.


### 3.3 Analyse the data {-}

Analyses the data using the procedures defined at points 3.1 and 3.2.
