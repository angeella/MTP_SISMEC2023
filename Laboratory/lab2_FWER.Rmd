...
a) Define now a procedure that considers also the endpoint % RT (i.e., 3 doses comparisons, 2 endpoints). One could use a parallel Gatekeeping procedure with two familes defined by the two endpoints

b) Is there any room for a further improvement of the procedure? if yes, make your proposal.
...
use procedure at point 3.1 at $\alpha$s levels reduced by $1/2$. As an example,  $H_{40}$ in each endpoints is tested at level $\alpha/2/2=\alpha/4$ etc.
...
##### point b) note that after rejection of H10_eps the alpha is not invested anymore, one could decide (apriori) to invest it in H40_eps, which surprisingly would lead to the rejection of H40_eps
