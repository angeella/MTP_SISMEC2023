...
\begin{itemize}
    ...
    \item \rbf{\textbf{Primary and secondary populations}}: \tetxbf{General} population versus \textbf{subgroups} of patients who are more likely to benefit from treatment
...
\end{center}
\begin{itemize}
    \item $p_A\leq \alpha$ implies rejection of (local) $H_{A}$ but also $H_{AB}$, $H_{AC}$ and $H_{ABC}$
    \item $p_B\leq \alpha$ implies rejection of (local) $H_{B}$ but also $H_{BC}$ ($H_{AC}$ and $H_{ABC}$ already rejected).
\end{itemize}
...
Family $1$ is a \textbf{\rbf{serial gatekeeper}} for Family $2$ $\rightarrow$ \tetxbf{All} hypotheses must be rejected in Family $1$ to proceed to Family $2$.
