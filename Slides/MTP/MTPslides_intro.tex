...
\multicolumn{1}{c}{\multirow{-2}{*}{Test}} & Not rejected & \multicolumn{1}{l|}{{\color[HTML]{9A0000} Type II error}}                                      & {\color[HTML]{3531FF} True negative}                                                    
\end{tabular}
\end{table}
...
\begin{overprint}
\only<1> {Under $H_0$, the p-value is a uniform random variable $U(0,1)$}
\only<2> {Type I Error: $\mathbb{P}(p\leq .05 |H_0)=.05$} % (or at least $\leq .05$)}
\end{overprint} 
\begin{overprint} 
...
\begin{overprint}
\only<1> {Under $H_1$, the p-value is stochastically smaller than a uniform random variable $U(0,1)$ (No test distortion)}
...
\midrule
\multicolumn{1}{c}{}                       & Rejected     & \multicolumn{1}{l}{{\color[HTML]{3166FF} $S$}}                                     & {\color[HTML]{9A0000} $V$ (false discoveries)}  &  $R$ \\
